...
def create_db():
    ...
    with sl.connect(database_name) as con:
        cur = con.cursor()
...
def insert_validator(public_key: str):
    """
    Inserts the validator with the public key passed into the database
    ...
    with sl.connect(database_name) as con:
        ...
        # insert validator
        cur.execute('INSERT OR IGNORE INTO validators(public_key) VALUES(?)', (public_key,))
...
def insert_validators(keys: list):
    ...
    """
    # iterate through the list, adding any validator that is not already in the database
    for key in keys:
        if not validator_exists(key):
            insert_validator(key)
...
def insert_relayer(name: str, endpoint: str):
    """
    Inserts the relayer with the name and endpoint passed into the database
    ...
    endpoint : str
        The endpoint of the relayer
    """
    with sl.connect(database_name) as con:
        cur = con.cursor()

        # insert relay
        cur.execute('INSERT OR IGNORE INTO relayers(name, endpoint) VALUES (?, ?)', (name, endpoint,))
...
def insert_relayers(relay_config: dict):
    ...
    relay_config : dict
        A dict where the key is the name of the relay and the value is the endpoint
    """
    # insert each relayer that does not exist yet in the database
    for name, endpoint in relay_config.items():
        if not relayer_exists(endpoint):
            insert_relayer(name, endpoint)

    # also insert 'Unknown'
    if not relayer_exists('N/A'):
        insert_relayer('Unknown', 'N/A')
